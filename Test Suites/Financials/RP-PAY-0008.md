...
| 5 | Verify buyer cart cleanup. | The current implementation deletes all `cart_items` rows for the webhook `buyer_id` after a paid event. This matches the live release behavior even though selected item metadata is also sent during checkout session creation. |
